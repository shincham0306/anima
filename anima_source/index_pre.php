...
// 投稿の表示の処理
  try {
    ...
    $post_count = $row['num'];
      ...
      $post_row = $stmt->fetchAll();
      // ユーザーデータの取得
      $u_st = $pdo->query("SELECT * FROM users");
      $user_row = $u_st->fetchAll();

      // コメントデータの取得
      $c_st = $pdo->query("SELECT * FROM comments");
      $comment_row = $c_st->fetchAll();
               ...
               <div class="container-fluid">
                   <div class="row">
                     ...
                     <?php for($i = 0; $i <= $post_count; $i++) :
                       ...
                       // ユーザーID一覧を取得
                       $column = array_column($user_row, 'user_id');
                       $key = array_search($user_id, $column);
                       $user_present_row = $user_row[$key];
                      ?>
                     <div class="col-sm-12 post_div">
                       <hr>
                       <img class="icon_image" src="<?php echo "./profile_pictures/".$user_present_row['user_id'].".".$user_present_row['pic_extension'] ?>">
                       <p class=""><?php echo $user_present_row['user_name'] ?></p><br>
                       <img class="postimage" src="<?php echo $present_post_row['media_path'] ?>" class="">
                       <h4 class="text-center"><?php echo $present_post_row['message'] ?></h4><br>
                       <p>コメント</p>
                       <?php
                       // コメントデータの取得
                       $com_column = array_column($comment_row,'post_id');
                       $comment_count = count(array_keys($com_column, $present_post_row['post_id']));
                       for($i = 0; $i <= $comment_count; $i++):

                        ?>
                  <?php endfor; ?>
                       <a class="text-right" href="comment.php?post_id=<?php echo $present_post_row['post_id'] ?>">コメントする</a>
                       <p class="text-right"><?php echo $present_post_row['created_at'] ?></p><br>
                     </div>
                   <?php endfor ; ?>
                   </div>
               </div>
